...
;;; funcion que extrae los simbolos de una base 
;;; de conocimiento sin repeticiones
...
;;; Funcion bfs que no considera los nodos ya visitados
      ...
      (let ((node (car path)))
        (if (eql node end) (reverse path) ; Si el nodo satisface el 
          ...
          (bfs end (append
                    ...
                    (new-paths path node net)) ;añadimos los nodos
                                               ;resultantes de la expasion
               (remove nil (mapcar #'(lambda (x) (remove-node node x))net))
                         ))))));; quitamos el nodo actual para evitar repeticiones

;;; Funcion auxliar que permite la eliminacion de los nodos ya visitados
...
;;; encuenta los nodos ya explorados
